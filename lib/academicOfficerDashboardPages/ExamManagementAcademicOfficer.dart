...
class _ExamManagementScreenState extends State<ExamManagementScreen>
        ...
        content: Column(
          ...
          children: [
            ListTile(
              ...
              title: const Text('Lock All Active Exams'),
              onTap: () {
                Navigator.pop(context);
                _bulkLockExams();
              },
  ...
  void _bulkLockExams() {
    setState(() {
      examSessions = examSessions.map((exam) {
        if (!exam.isLocked && !exam.isOverdue) {
          return exam.copyWith(isLocked: true);
        }
        return exam;
      }).toList();
    });
    _showSuccessSnackBar('All active exams have been locked');
  }
